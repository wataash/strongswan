...
static bool in_debugger()
{
	...
	// /proc/self/status:
	// TracerPid:	0
	(void)snprintf(buf, sizeof(buf), "grep -P -q '^TracerPid:\t0$' /proc/%jd/status", (intmax_t)getpid());
	rc = system(buf);
	if (rc == 0) {
		return false;
	}
	(void)snprintf(buf, sizeof(buf), "grep -P '^TracerPid:\t' /proc/%jd/status >&2", (intmax_t)getpid());
	(void)system(buf);
	return true;
}
#endif // WATAASH_HACK
